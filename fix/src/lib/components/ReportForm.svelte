<script lang='ts'>
    ...
    import dailyTasks from '../../../../src/lib/DailyTasks.js';
    import globalTasks from '../../../../src/lib/GlobalTasks.js';
    ...
    function updateGeneratedMessage(taskName: string = '', taskExp: number = 0, taskCoins: number = 0) {
        if (taskName) {
            ...
            const finalCoins = Math.round(taskCoins * currentBonusMultiplier);
...
<div class="flex flex-col bg-white border border-gray-200 shadow-2xs rounded-xl mb-4">
    <div class="p-4 md:p-7">
        ...
        <div class="flex items-center mt-2 text-gray-500 mb-2 gap-x-4">
            <p class="whitespace-nowrap">Игровой ник:</p>
            <input
                type="text"
                bind:value={$nickname}
                ...
                on:input={handleNicknameChange}
            />
        </div>

        <div class="flex items-center mt-2 text-gray-500 mb-2 gap-x-4">
            <p class="whitespace-nowrap">Бонус:</p>
            <select
                bind:value={$selectedBonusKey}
                class="py-2 px-4 block border-gray-200 rounded-lg sm:text-sm focus:border-blue-500 focus:ring-blue-500 disabled:opacity-50 disabled:pointer-events-none w-full"
                on:change={() => updateSelectedBonusKey($selectedBonusKey)}
            >
                {#each Object.keys(bpBonuses) as bonusKey}
                    <option value={bonusKey}>{bonusKey}</option>
                {/each}
                <option value="Без бонуса">Без бонуса</option>
            </select>
        </div>
